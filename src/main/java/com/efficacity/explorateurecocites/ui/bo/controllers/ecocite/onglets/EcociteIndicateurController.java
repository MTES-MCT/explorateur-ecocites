    ...
    public String ajoutToEcocite(Locale locale, Model model, @RequestBody AjoutIndicateurObjetForm object) {
        ...
        if(model.containsAttribute("indicateurLectureSeule") && !(boolean) model.asMap().get("indicateurLectureSeule")) {
            ...
            if(CustomValidator.isEmpty(object.getUnite()) || DEFAULT_VALUE_SELECT.equals(object.getUnite())){
                model.addAttribute("general_error", messageSourceService.getMessageSource().getMessage("error.attribut.notNull", null, locale));
                hasError = true;
            }
            ...
            if(assoIndicateurEcociteBean != null){
                model.addAttribute("general_error", "Cette association existe déjà");
                hasError = true;
            }
            ...
            if(!hasError) {
                ...
                try {
                    ...
                    model.addAttribute("general_error", messageSourceService.getMessageSource().getMessage("error.technical", null, locale));
                    return "bo/ecocites/ongletEdition/indicateurAjout";
                }
                model.addAttribute("assoIndicateurEcocite", assoIndicateurEcociteBean);
            }
        } else {
            model.addAttribute("general_error", messageSourceService.getMessageSource().getMessage("error.user.right", null, locale));
        }
        return "bo/ecocites/ongletEdition/indicateurAjout";
    }
