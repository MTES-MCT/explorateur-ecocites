...
//        return repository.findAll(where(hasIdObjet(ecociteId)).and(belongToEcocite()));
    ...
    private AssoObjetContact createOnePrimaryContactForEcocite(final Contact contact, final Ecocite ecocite) {
        repository.delete(repository.findAll(where(hasIdObjet(ecocite.getId())).and(belongToEcocite()).and(hasIdContact(contact.getIdEcocite())).and(isContactPrincipal())));
    ...
    private AssoObjetContact createOnePrimaryContactForAction(final Contact contact, final Action action) {
        repository.delete(repository.findAll(where(hasIdObjet(action.getId())).and(belongToAction()).and(hasIdContact(contact.getIdEcocite())).and(isContactPrincipal())));
        AssoObjetContact bean = new AssoObjetContact();
        bean.setIdContact(contact.getId());
    ...
    public void deleteAssoActionContact(final Long idContact, final Long idAction) {
        repository.delete(repository.findAll(where(hasIdObjet(idAction)).and(belongToAction()).and(hasIdContact(idContact))));
    }

    public void deleteAssoEcociteContact(final Long idContact, final Long idEcocite) {
        repository.delete(repository.findAll(where(hasIdObjet(idEcocite)).and(belongToEcocite()).and(hasIdContact(idContact))));
    }

    public void deleteByAction(final Long idAction) {
        repository.delete(repository.findAll(where(hasIdObjet(idAction)).and(belongToAction())));
    }

    public void deleteByEcocite(final Long idEcocite) {
        repository.delete(repository.findAll(where(hasIdObjet(idEcocite)).and(belongToEcocite())));
    }

    public void deleteByContact(final Long idContact) {
        repository.delete(repository.findAll(hasIdContact(idContact)));
    }

    public void deleteAssoEcociteContactPrincipal(final Long idEcocite) {
        repository.delete(repository.findAll(where(hasIdObjet(idEcocite)).and(belongToEcocite()).and(isContactPrincipal())));
    }

    public void deleteAssoActionContactPrincipal(final Long idAction) {
        repository.delete(repository.findAll(where(hasIdObjet(idAction)).and(belongToAction()).and(isContactPrincipal())));
    }
