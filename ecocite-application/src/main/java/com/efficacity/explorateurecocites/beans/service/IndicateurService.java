...
public class IndicateurService extends CrudEntityService<IndicateurRepository, Indicateur, Long> {
    ...
    public Indicateur findOneByIdVisibleForUser(Long id, String echelle, List<String> natures, JwtUser user) {
        if (natures != null && !natures.isEmpty()) {
            return repository.findOne(where(isValideAndPublieOrCreateur(user.getEmail())).and(hasEchelle(echelle)).and(hasNatureIn(natures)).and(hasId(id))).orElse(null);
        } else {
            return repository.findOne(where(isValideAndPublieOrCreateur(user.getEmail())).and(hasEchelle(echelle)).and(hasId(id))).orElse(null);
        }
    }

    public IndicateurBean findOneIndicateur(Long idIndicateur){
        return toIndicateurBean(repository.findById(idIndicateur).orElse(null));
    }
    ...
    public IndicateurBean findOneIndicateurValideVisibleForUser(Long idIndicateur, JwtUser user){
        return toIndicateurBean(repository.findOne(where(isValideAndPublieOrCreateur(user.getEmail())).and(hasId(idIndicateur))).orElse(null));
    }
    ...
    public String delete(Long idIndicateur, Locale locale) {
        ...
        if(assoIndicateurObjetService.countByIdIndicateur(idIndicateur) > 0){
            ...
            repository.deleteById(idIndicateur);
            return "";
        }
    }
    ...
    */
    public List<Indicateur> getListFiltre(Map<String,String> filtres, JwtUser user) {
        Specification<Indicateur> spec = null;
        if(filtres != null){
    ...
    public Indicateur updateIndicateur(final Long id, final IndicateurForm indicateurForm, final JwtUser user) {
        Indicateur bean = repository.findById(id).orElseThrow(() -> new EntityNotFoundException());;
    ...
    public IndicateurBean findOneVisibleIndicateur(final Long idIndicateur) {
        try {
            return toIndicateurBean(repository.findOne(where(hasId(idIndicateur)).and(isValide())).orElse(null));
        } catch (Exception e) {
            return null;
        }
    }
