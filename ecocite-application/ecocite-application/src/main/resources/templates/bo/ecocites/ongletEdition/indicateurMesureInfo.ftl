...
<tr id="row-${type}-${object.id?c}">
    <td class="center-table-cell">${object.dateSaisie?datetime("yyyy-MM-dd")?string["dd/MM/yyyy"]}</td>
    <#if displayWithMap?? && displayWithMap && selectMap?? && object.valeur??>
        <td class="center-table-cell">${selectMap[object.valeur]!""}</td>
    <#else>
        <td class="center-table-cell">${object.valeur!""}</td>
    </#if>
    <td class="center-table-cell ft-s-11">${assoIndicateurEcocite.unite}</td>
    <#--TODO save auto -->
