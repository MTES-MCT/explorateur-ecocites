...
public class SaveAttributController {
    ...
    public ResponseEntity saveAttribut(@RequestBody @Valid BeanSaveAttribut object, Model model,
        ...
        if(!useCanModify(model, object)){
            result.reject(ApplicationConstants.GENERAL_ERROR, messageSourceService.getMessageSource().getMessage("error.user.right", null, locale));
            ...
            case "ReponseEvaluation":
                ...
                if (!result.hasErrors()) {
                    try {
                        ...
                        result.reject(ApplicationConstants.GENERAL_ERROR, messageSourceService.getMessageSource().getMessage(ApplicationConstants.ERROR_TECHNICAL, null, locale));
                    }
                }
                break;
            case "Action":
                if (Objects.equals(object.getAttributId(), "typeFinancement")) {
                    ...
                    if(CustomValidator.isEmpty(value)){
                        result.rejectValue(ApplicationConstants.ATTRIBUT_VALUE,"error.attribut.notNull", messageSourceService.getMessageSource().getMessage("error.attribut.notNull", null, locale));
                    ...
                    if (Objects.equals(object.getAttributId(), "numeroAction") && actionService.countByNumeroDifferentFromAction(object.getAttributValue(),
                        ...
                        result.rejectValue(ApplicationConstants.ATTRIBUT_VALUE, "error.attribut.unique", "Ce numéro d'action est déjà utilisé, merci de vérifier sa validité");
                    }
                    if (!result.hasErrors()) {
                        try {
